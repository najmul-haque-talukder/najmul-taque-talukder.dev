...
function BlockViewerToolbar() {
  const { setView, item, resizablePanelRef, setIframeKey } = useBlockViewer() as any
...
function BlockViewerView() {
  const { resizablePanelRef } = useBlockViewer() as any
...
function BlockViewerIframe({ className }: { className?: string }) {
  const { iframeKey, item } = useBlockViewer() as any
...
function BlockViewerCode() {
  const { highlightedFiles, activeFile } = useBlockViewer() as any

  const file = useMemo(() => {
    return highlightedFiles?.find((file: any) => file.target === activeFile)
...
function BlockViewerFileTree() {
  const { tree } = useBlockViewer() as any
...
function Tree({ item, index }: { item: FileTree; index: number }) {
  const { activeFile, setActiveFile } = useBlockViewer() as any
...
function BlockCopyCodeButton() {
  const { item, activeFile } = useBlockViewer() as any
...
function BlockViewerMobile() {
  const { item } = useBlockViewer() as any
